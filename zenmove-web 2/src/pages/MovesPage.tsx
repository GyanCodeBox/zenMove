...
export default function MovesPage() {
  ...
  useEffect(() => {
    ...
    api.moves.list(page, statusFilter || undefined)
      .then((res) => { setMoves(res.data); setTotal(res.total) })
      .catch(console.error)
      .finally(() => setLoading(false))
  }, [page, statusFilter])

  // Stats
  const active   = moves.filter(m => !['completed','disputed'].includes(m.status)).length
  const inTransit = moves.filter(m => m.status === 'in_transit').length
